...
function AI() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch(`https://opexdijital.up.railway.app/api/upload`, {
        method: "POST",
        body: formData,
      });
  ...
  useEffect(() => {
    async function checkStatus() {
      try {
        const res = await fetch(`https://opexdijital.up.railway.app/api/ai/status`);
        const data = await res.json();
  ...
  async function sendQuestion(
    ...
    try {
      ...
      const response = await fetch(
        `https://opexdijital.up.railway.app/api/ai`,
        {
          method: "POST",
