...
export const getAbsentDays = (r: AttendanceRow): number => {
  ...
  const absentKey = keys.find(k => 
    (k.includes("KATILIMOLMAYAN") || k.includes("DEVAMSIZ")) && !k.includes("MAZERET")
  );
...
export const isAttended = (r: AttendanceRow) => {
  ...
  const katilimKey = keys.find(k => 
    (k.includes("KATILIM") || k === "KATIL" || k.includes("DURUM")) && 
...
export const isLate = (r: AttendanceRow) => {
  const keys = Object.keys(r);
  const lateKey = keys.find(k => k.includes("GEÇKATIL") || k.includes("GECKATIL") || k.includes("GECİKME"));
...
export const isExcused = (r: AttendanceRow) => {
  ...
  const excusedKey = keys.find(k => 
    (k.includes("MAZERET") || k.includes("İZİN") || k.includes("IZIN")) && 
...
export const getHafta = (r: AttendanceRow): number | null => {
  const keys = Object.keys(r);
  const haftaKey = keys.find(k => k.includes("HAFTA") || k.includes("WEEK"));
...
export const getBolum = (r: AttendanceRow): string => {
  const keys = Object.keys(r);
  const bolumKey = keys.find(k => k.includes("BÖLÜM") || k.includes("BOLUM") || k.includes("HAT") || k.includes("ALAN"));
